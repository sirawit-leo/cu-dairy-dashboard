...
| `feed.html` | **Feed evaluation** — 11 stacked sections plus a per-cow record at the foot; clicking any cow on any chart or any cow number in any table opens it. Sections 1–9 follow the order and the chart-or-table choice of the farm's own Feeding-management report of 22 Aug 2569 (`raw/feed/สรุปฝ่าย Feeding management (22 ส.ค. 2569).html`), **including its own ordering, where its §6 is shown before its §5**; sections 10–11 are ours and have no counterpart there. Every figure is rebuilt from the master — nothing is copied from that report — and each place the two disagree carries a *Where ours and the department's differ* block naming both values and the reason. Every chart is SVG written at build time, so the pictures do not depend on JavaScript. **Added 2026-08-23, restructured 2026-08-23.** |
